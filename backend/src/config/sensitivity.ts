 ...
 * label to "High". There's no user-selectable sensitivity tier - the app always evaluates
 * against DEFAULT_SENSITIVITY below - but the three calibrated tiers are kept here so that
 * threshold can be changed in one place, and reused if per-user selection comes back later.
 *
...
// The single threshold everything (fastest/quietest, crowd alert) is evaluated against now that
// there's no user-selectable tier - see routingService.ts planRoutes.
